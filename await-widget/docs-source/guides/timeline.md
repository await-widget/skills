...
`widgetTimeline()` is optional. Use it when a widget needs dated entry data or scheduled visual changes.

A timeline is the schedule behind the widget snapshot. Each entry has a `date`; the system pulls one entry at that date and passes it into `widget(entry)`.
